	...
	var a = {
		init : function (d) {
			...
			return this.each(function () {
				...
				n.after("<div class='mCSB_scrollTools' style='position:relative;'><div class='mCSB_draggerContainer'><div class='mCSB_dragger' style='position:absolute;' oncontextmenu='return false;'><div class='mCSB_dragger_bar' style='position:relative;'></div></div><div class='mCSB_draggerRail'></div></div></div>");
		...
		scrolling : function (h, p, m, j, z, e, C, v) {
			var k = b(this);
			if (!k.data("bindEvent_scrollbar_drag")) {
				...
				if (b.support.pointer || b.support.msPointer) {
					j.bind(B, function (J) {
						...
						var I = b(this),
						L = I.offset(),
						H = J.originalEvent.pageX - L.left,
						K = J.originalEvent.pageY - L.top;
						if (H < I.width() && H > 0 && K < I.height() && K > 0) {
							n = K;
							o = H
						}
					});
					b(document).bind(w + "." + k.data("mCustomScrollbarIndex"), function (J) {
						J.preventDefault();
						if (k.data("on_drag")) {
							var I = j,
							L = I.offset(),
					...
					j.bind("mousedown touchstart", function (J) {
						J.preventDefault();
						...
						if (J.type === "touchstart") {
							var K = J.originalEvent.touches[0] || J.originalEvent.changedTouches[0];
							H = K.pageX - M.left;
							L = K.pageY - M.top
						} else {
							k.data({
						...
						if (H < I.width() && H > 0 && L < I.height() && L > 0) {
							n = L;
							o = H
						}
					...
					b(document).bind("mousemove." + k.data("mCustomScrollbarIndex"), function (J) {
						if (k.data("on_drag")) {
							var I = j,
							L = I.offset(),
							H = J.pageX - L.left,
			...
			function F(I, J, K, H) {
				if (k.data("horizontalScroll")) {
					k.mCustomScrollbar("scrollTo", (j.position().left - (J)) + H, {
						moveDragger : true,
						trigger : "internal"
					})
				} else {
					k.mCustomScrollbar("scrollTo", (j.position().top - (I)) + K, {
						moveDragger : true,
						trigger : "internal"
					})
				}
			...
			if (b.support.touch && k.data("contentTouchScroll")) {
				if (!k.data("bindEvent_content_touch")) {
					...
					p.bind("touchmove", function (x) {
						...
						s = l.pageY - r.top;
						if (k.data("horizontalScroll")) {
							k.mCustomScrollbar("scrollTo", G - u, {
								trigger : "internal"
							})
						} else {
							k.mCustomScrollbar("scrollTo", E - s, {
								trigger : "internal"
							})
						}
					})
			...
			if (!k.data("bindEvent_scrollbar_click")) {
				m.bind("click", function (H) {
					var x = (H.pageY - m.offset().top) * k.data("scrollAmount"),
					y = b(H.target);
					if (k.data("horizontalScroll")) {
						x = (H.pageX - m.offset().left) * k.data("scrollAmount")
					}
			...
			if (k.data("mouseWheel")) {
				if (!k.data("bindEvent_mousewheel")) {
					h.bind("mousewheel", function (J, L) {
						var I,
						H = k.data("mouseWheelPixels"),
						...
						if (H === "auto") {
							H = 100 + Math.round(k.data("scrollAmount") / 2)
						}
						if (k.data("horizontalScroll")) {
							K = j.position().left;
							y = m.width() - j.width();
			...
			if (k.data("scrollButtons_enable")) {
				if (k.data("scrollButtons_scrollType") === "pixels") {
					if (k.data("horizontalScroll")) {
						v.add(C).unbind("mousedown touchstart MSPointerDown pointerdown mouseup MSPointerUp pointerup mouseout MSPointerOut pointerout touchend", i, g);
						k.data({
							bindEvent_buttonsContinuous_x : false
						});
					...
					if (k.data("horizontalScroll")) {
						...
						if (!k.data("bindEvent_buttonsContinuous_x")) {
							...
							C.bind("mousedown touchstart MSPointerDown pointerdown", function (y) {
								y.preventDefault();
								var x = A();
								k.data({
							...
							C.bind("mouseup touchend MSPointerUp pointerup mouseout MSPointerOut pointerout", g);
							k.data({
				...
				if (!k.data("bindEvent_focusin")) {
					h.bind("focusin", function () {
						...
						if (x.is("input,textarea,select,button,a[tabindex],area,object")) {
							...
							y = x.position().top,
							H = h.height() - x.outerHeight();
							if (k.data("horizontalScroll")) {
								I = p.position().left;
								y = x.position().left;
								H = h.width() - x.outerWidth()
							}
							if (I + y < 0 || I + y > H) {
						...
						if (o < 0) {
							...
							if (o >= i.height() - g.height()) {
								...
								if (!k) {
									...
									$( ".mCSB_buttonDown" ).css( "opacity", "0.2" );	
									l = true
								}
							} else {
								d = -d
